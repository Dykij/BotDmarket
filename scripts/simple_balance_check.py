...
class DirectBalanceChecker:
    ...
    async def check_balance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Ошибка при запросе через новый эндпоинт: {str(e)}")
        ...
        try:
            ...
            logger.error(f"Ошибка при запросе через старый эндпоинт: {str(e)}")
    ...
    def parse_balance(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "usd" in response and isinstance(response["usd"], dict) and "amount" in response["usd"]:
            ...
            if "balance" in response and isinstance(response["balance"], dict) and "usd" in response["balance"]:
            ...
            logger.error(f"Ошибка при парсинге баланса: {str(e)}")
...
async def main():
    ...
    print("\n" + "="*60)
    print(f"РЕЗУЛЬТАТЫ ПРОВЕРКИ БАЛАНСА DMARKET")
    print("="*60)
