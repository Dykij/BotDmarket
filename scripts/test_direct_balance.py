    ...
    if response and isinstance(response, dict):
        ...
        if "usd" in response:
            try:
                ...
                if isinstance(response["usd"], str):
                    ...
                    logger.info(
                        f"Обрабатываем баланс из строки: {response['usd']} -> {usd_value} центов"
                    )
                    return {"usd": {"amount": usd_value}}
                # Если это число
                if isinstance(response["usd"], (int, float)):
                    usd_value = float(response["usd"]) * 100  # преобразуем в центы
                    logger.info(
                        f"Обрабатываем баланс из числа: {response['usd']} -> {usd_value} центов"
                    )
                    return {"usd": {"amount": usd_value}}
                # Если это уже нужный формат с вложенным объектом
                if isinstance(response["usd"], dict) and "amount" in response["usd"]:
                    logger.info(
                        f"Баланс уже в правильном формате: {response['usd']['amount']} центов"
                    )
                    return response
        ...
        if "usdAvailableToWithdraw" in response:
            try:
                ...
                logger.info(
                    f"Баланс из usdAvailableToWithdraw: {response['usdAvailableToWithdraw']} -> {usd_value} центов"
                )
                return {"usd": {"amount": usd_value}}
            except (ValueError, TypeError) as e:
...
async def test_balance():
    ...
    try:
        ...
        logger.error(f"Ошибка при получении баланса: {e!s}")
