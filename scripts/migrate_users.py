...
def load_user_data(filepath: str) -> Optional[Dict[str, Any]]:
    """
...
def create_backup(user_files: Dict[str, str], backup_dir: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Ошибка при создании резервных копий: {str(e)}")
        return False
...
def migrate_user_data(user_files: Dict[str, str], output_path: str) -> Dict[str, Any]:
    ...
    if "profiles" in user_files:
        try:
            ...
            logger.error(f"Ошибка при загрузке существующих профилей: {str(e)}")
