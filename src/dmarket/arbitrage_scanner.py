...
class ArbitrageScanner:
    ...
    async def scan_multiple_games(
        ...
        for game in games:
            try:
                ...
                task = asyncio.create_task(self.scan_game(
                    game=game,
                    mode=mode,
                ...
                logger.error(f"Ошибка при создании задачи для игры {game}: {str(e)}")
                results[game] = []
    ...
    async def check_user_balance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in balance_response or not balance_response.get("usd"):
                ...
                if "unauthorized" in str(error_message).lower() or "авторизации" in str(error_message).lower():
    ...
    async def _get_current_item_data(self, item_id: str, game: str, api_client: DMarketAPI) -> Optional[Dict[str, Any]]:
        """
    ...
    async def _purchase_item(self, item_id: str, max_price: float, api_client: DMarketAPI) -> Dict[str, Any]:
        """
    ...
    async def _list_item_for_sale(self, item_id: str, price: float, api_client: DMarketAPI) -> Dict[str, Any]:
        """
