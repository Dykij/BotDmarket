...
class ArbitrageScanner:
        ...
        try:
            ...
            if price_from is None and price_to is None:
                try:
                    ...
                    logger.warning(f"Ошибка при использовании встроенных функций арбитража: {e!s}")
                    items = []
            ...
            try:
                ...
                items.extend(
                    self._standardize_items(items_from_trader, game, min_profit, max_profit)
                )
            except Exception as e:
    ...
    async def scan_multiple_games(
        ...
        for game in games:
            try:
                ...
                logger.error(f"Ошибка при создании задачи для игры {game}: {e!s}")
                results[game] = []
    ...
    async def auto_trade_items(
        ...
        for item in sorted_items:
            ...
            try:
                ...
                logger.error(f"Ошибка при торговле предметом '{item.get('title', '')}': {e!s}")
                trades_count += 1
    ...
    async def _get_current_item_data(
        self, item_id: str, game: str, api_client: DMarketAPI
    ) -> dict[str, Any] | None:
    ...
    async def _purchase_item(
        self, item_id: str, max_price: float, api_client: DMarketAPI
    ) -> dict[str, Any]:
    ...
    async def _list_item_for_sale(
        self, item_id: str, price: float, api_client: DMarketAPI
    ) -> dict[str, Any]:
