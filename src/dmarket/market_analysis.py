...
from src.utils.performance import profile_performance, cached
...
async def analyze_price_changes(
    ...
    try:
        ...
        logger.error(f"Ошибка при анализе изменений цен: {e}")
        return []
    finally:
...
async def find_trending_items(
    ...
    try:
        ...
        logger.error(f"Ошибка при поиске трендовых предметов: {e}")
        return []
    finally:
        if close_client and hasattr(dmarket_api, "_close_client"):
            try:
...
async def analyze_market_volatility(
    ...
    try:
        ...
        historical_24h = await _get_historical_prices(game=game, period_hours=24, dmarket_api=dmarket_api)
        historical_7d = await _get_historical_prices(game=game, period_hours=24*7, dmarket_api=dmarket_api)
        ...
        logger.error(f"Ошибка при анализе волатильности рынка: {e}")
        return []
    finally:
        if close_client and hasattr(dmarket_api, "_close_client"):
            try:
...
async def generate_market_report(
    ...
    try:
        ...
        tasks = [
            analyze_price_changes(game=game, period="24h", dmarket_api=dmarket_api, limit=10),
        ...
        report = {
            ...
            "market_summary": {
                ...
                "market_volatility_level": _calculate_market_volatility_level(results[2]),
        ...
        logger.error(f"Ошибка при создании отчета о рынке: {e}")
        return {
            "game": game,
            "timestamp": int(time.time()),
            "error": str(e),
...
def _extract_price_from_item(item: Dict[str, Any]) -> float:
    ...
    try:
        ...
        if "bestPrice" in item:
            ...
            elif isinstance(best_price, (int, float)):
...
def _calculate_market_volatility_level(volatile_items: List[Dict[str, Any]]) -> str:
    ...
    avg_volatility = sum(item.get("volatility_score", 0) for item in volatile_items) / len(volatile_items)
