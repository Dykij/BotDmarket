...
_arbitrage_cache: dict[tuple[str, str, float, float], tuple[list[SkinResult], float]] = {}
...
async def fetch_market_items(
    ...
    try:
        ...
        logger.error(f"Ошибка при получении предметов: {e!s}")
        return []
...
async def _find_arbitrage_async(
    ...
    cache_key = (game, f"{min_profit}-{max_profit}", price_from or 0, price_to or float("inf"))
...
class ArbitrageTrader:
    ...
    def __init__(
        ...
        self.api_url = api_url or os.environ.get("DMARKET_API_URL", "https://api.dmarket.com")
    ...
    async def check_balance(self) -> tuple[bool, float]:
        ...
        try:
            ...
            logger.error(f"Ошибка при проверке баланса: {e!s}")
            return False, 0.0
    ...
    async def _check_trading_limits(self, trade_value: float) -> bool:
        ...
        if self.daily_traded + trade_value > self.daily_limit:
            logger.warning(
                f"Сделка на ${trade_value:.2f} превысит дневной " f"лимит ${self.daily_limit:.2f}",
            )
            return False
    ...
    async def execute_arbitrage_trade(
        ...
        try:
            ...
            logger.error(f"Ошибка при выполнении сделки: {e!s}")
    ...
    async def _auto_trading_loop(
        ...
        while self.active:
            try:
                ...
                if not has_funds:
                    logger.warning(
                        f"Недостаточно средств для торговли: ${balance:.2f}, " f"ожидание 5 минут",
                    )
                ...
                logger.error(f"Ошибка в цикле автоторговли: {e!s}")
    ...
    async def get_current_item_data(
        self, item_id: str, game: str = "csgo"
    ) -> dict[str, Any] | None:
    ...
    async def purchase_item(
        self, item_id: str, max_price: float, dmarket_api: DMarketAPI | None = None
    ) -> dict[str, Any]:
    ...
    async def list_item_for_sale(
        self, item_id: str, price: float, dmarket_api: DMarketAPI | None = None
    ) -> dict[str, Any]:
...
async def find_arbitrage_opportunities(
    ...
    if game not in GAMES:
        logger.warning(f"Неизвестная игра: {game}. Доступные игры: {', '.join(GAMES.keys())}. Используется csgo.")
        game = "csgo"

    if mode not in MIN_PROFIT_PERCENT and mode not in ["low", "medium", "high", "boost", "pro"]:
        logger.warning(
    ...
    try:
        ...
        for item_name, items in grouped_items.items():
            ...
            commission_percent = _calculate_commission(item_rarity, item_type, item_popularity, game)
            ...
            for item in items[1:]:
                ...
                if profit_percent >= min_profit and net_profit > 0:
                    ...
                    if buy_id:
                        opportunity["buy_link"] = f"https://dmarket.com/ingame-items/item-list/{game}-skins?userOfferId={buy_id}"
                    if sell_id:
                        opportunity["sell_link"] = f"https://dmarket.com/ingame-items/item-list/{game}-skins?userOfferId={sell_id}"
...
def _calculate_commission(rarity: str, item_type: str, popularity: float, game: str) -> float:
    ...
    if rarity.lower() in ["covert", "extraordinary", "contraband", "ancient", "mythical", "immortal", "arcana"]:
...
def _get_arbitrage_cache(cache_key: tuple) -> Optional[List[Dict[str, Any]]]:
...
def _save_arbitrage_cache(cache_key: tuple, items: List[Dict[str, Any]]) -> None:
    ...
    if len(_arbitrage_cache) > 20:
        ...
        oldest_keys = sorted(_arbitrage_cache.keys(), 
                            key=lambda k: _arbitrage_cache[k][1])[:5]
...
def _group_items_by_name(items: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
