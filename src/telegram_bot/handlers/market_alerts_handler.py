...
async def alerts_command(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if user_subscriptions:
            control_row.append(
                InlineKeyboardButton(
                    "🔕 Отписаться от всех", callback_data="alerts:unsubscribe_all"
                ),
            )
        ...
        if user_subscriptions or price_alerts:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        "⚙️ Настройки уведомлений", callback_data="alerts:settings"
                    ),
                ]
            )
...
async def show_user_alerts_list(query, user_id: int) -> None:
    ...
    if not alerts:
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton(
                    "⬅️ Назад к уведомлениям", callback_data="alerts:back_to_alerts"
                ),
            ],
        ]
