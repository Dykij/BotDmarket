...
class UserProfileManager:
    ...
    def _init_encryption(self) -> None:
        ...
        if ENCRYPTION_KEY_FILE.exists():
            ...
            try:
                os.chmod(ENCRYPTION_KEY_FILE, 0o600)  # Только чтение и запись для владельца
    ...
    def load_profiles(self) -> None:
        ...
        try:
            ...
            logger.error(f"Ошибка при загрузке профилей пользователей: {e}")
            self._profiles = {}
    ...
    def save_profiles(self, force: bool = False) -> None:
        ...
        try:
            ...
            for user_id, profile in self._profiles.items():
                ...
                if "api_keys" in profile_copy and isinstance(profile_copy["api_keys"], dict):
                    encrypted_keys = {}
...
async def check_user_access(update: Update, context: ContextTypes.DEFAULT_TYPE, feature: str) -> bool:
...
async def get_api_keys(user_id: int) -> Tuple[Optional[str], Optional[str]]:
...
async def set_api_keys(user_id: int, public_key: str, secret_key: str) -> bool:
    ...
    try:
        ...
        profile_manager.update_profile(user_id, {"api_keys_info": profile["api_keys_info"]})
