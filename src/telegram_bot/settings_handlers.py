...
def save_user_profiles():
    ...
    try:
        ...
        user_profiles_file = os.path.join(
            os.path.dirname(__file__), "user_profiles.json"
        )
...
async def settings_callback(update: Update, context: CallbackContext) -> None:
    ...
    if data == "settings":
        ...
        await query.edit_message_text(
            text=settings_text, reply_markup=keyboard
        )
        ...
        language_text = get_localized_text(
            user_id, "language", lang=lang_display_name
        )
        keyboard = get_language_keyboard(current_language)
        
        await query.edit_message_text(
            text=language_text, reply_markup=keyboard
        )
        ...
        if lang_code in LANGUAGES:
            ...
            confirmation_text = get_localized_text(
                user_id, "language_set", lang=lang_display
            )
            
            keyboard = get_back_to_settings_keyboard()
            await query.edit_message_text(
                text=confirmation_text, reply_markup=keyboard
            )
        else:
            ...
            await query.edit_message_text(
                text=error_text, reply_markup=keyboard
            )
        ...
        keyboard = get_back_to_settings_keyboard()
        await query.edit_message_text(
            text=api_text, reply_markup=keyboard
        )
        ...
        await query.edit_message_text(
            text=limits_text, reply_markup=keyboard
        )
        ...
        welcome_text = get_localized_text(
            user_id, "welcome", user=query.from_user.mention_html()
        )
