...
async def handle_sales_history_callback(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logger.exception(f"Ошибка при получении истории продаж: {str(e)}")
        await query.edit_message_text(
...
async def handle_refresh_sales_callback(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        # Обработка прочих ошибок
        logger.exception(f"Ошибка при обновлении анализа продаж: {str(e)}")
        await query.edit_message_text(
            f"❌ Произошла ошибка: {str(e)}",
            parse_mode='Markdown'
        )
...
async def handle_refresh_liquidity_callback(update: Update, context: CallbackContext) -> None:
    ...
    update.callback_query.data = update.callback_query.data.replace("refresh_liquidity:", "liquidity:")
...
async def handle_all_arbitrage_sales_callback(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        # Обработка прочих ошибок
        logger.exception(f"Ошибка при получении всех арбитражных возможностей: {str(e)}")
        await query.edit_message_text(
            f"❌ Произошла ошибка: {str(e)}",
            parse_mode='Markdown'
        )
    ...
    try:
        ...
        for i, item in enumerate(items, 1):
            ...
            if i == 15 and len(items) > 15:
                formatted_message += (
                    f"_Показаны 15 из {len(items)} предметов._\n\n"
                )
                break
        ...
        # Обработка прочих ошибок
        logger.exception(f"Ошибка при получении подробной статистики объема продаж: {str(e)}")
        await query.edit_message_text(
            f"❌ Произошла ошибка: {str(e)}",
            parse_mode='Markdown'
        )
