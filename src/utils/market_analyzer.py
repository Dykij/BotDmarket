...
class MarketAnalyzer:
    ...
    async def analyze_price_history(
        ...
        if not all(timestamps[i] <= timestamps[i+1] for i in range(len(timestamps)-1)):
            combined = sorted(zip(timestamps, prices), key=lambda x: x[0])
        ...
        if len(prices) > 1:
            ...
            for i, ts in enumerate(timestamps):
                if abs(ts - day_ago) < abs(timestamps[prices.index(day_ago_price)] - day_ago):
                    day_ago_price = prices[i]
                if abs(ts - week_ago) < abs(timestamps[prices.index(week_ago_price)] - week_ago):
    ...
    def _detect_patterns(
        ...
        if self._is_breakout(prices):
            patterns.append({
                ...
                "confidence": self._calculate_pattern_confidence(prices, PATTERN_BREAKOUT),
        ...
        if self._is_reversal(prices):
            ...
            patterns.append({
                ...
                "direction": rev_type,
                "confidence": self._calculate_pattern_confidence(prices, PATTERN_REVERSAL),
        ...
        if self._is_fomo(prices):
            patterns.append({
                "type": PATTERN_FOMO,
                "confidence": self._calculate_pattern_confidence(prices, PATTERN_FOMO),
        ...
        if self._is_panic(prices):
            patterns.append({
                "type": PATTERN_PANIC,
                "confidence": self._calculate_pattern_confidence(prices, PATTERN_PANIC),
        ...
        if self._is_bottoming(prices):
            patterns.append({
                ...
                "confidence": self._calculate_pattern_confidence(prices, PATTERN_BOTTOMING),
        ...
        if self._is_topping(prices):
            patterns.append({
                ...
                "confidence": self._calculate_pattern_confidence(prices, PATTERN_TOPPING),
    ...
    def _is_breakout(self, prices: List[float]) -> bool:
        ...
        if last_price > range_max + (range_size * 0.05):
            return True
        if last_price < range_min - (range_size * 0.05):
            return True
    ...
    def _calculate_pattern_confidence(
        ...
        if pattern_type == PATTERN_BREAKOUT:
            ...
            if range_size > 0:
                ...
                breakout_strength = abs(prices[-1] - (range_max if prices[-1] > range_max else range_min)) / range_size
...
async def analyze_market_opportunity(
    ...
    if analysis["support_level"] is not None:
        support_distance = (current_price - analysis["support_level"]) / current_price if current_price > 0 else 0
        if support_distance < 0.05 and support_distance > 0:
            # Price near support - potential buy
            opportunity_score += 20
            reasons.append(f"Price near support level (${analysis['support_level']:.2f})")
    
    if analysis["resistance_level"] is not None:
        resistance_distance = (analysis["resistance_level"] - current_price) / current_price if current_price > 0 else 0
        if resistance_distance < 0.05 and resistance_distance > 0:
            # Price near resistance - potential sell
            opportunity_score += 20
            reasons.append(f"Price near resistance level (${analysis['resistance_level']:.2f})")
    ...
    if abs(analysis["price_change_24h"]) > 15:
        ...
        if analysis["price_change_24h"] > 0:
            ...
            reasons.append(f"Price down {abs(analysis['price_change_24h']):.1f}% in 24h")
    ...
    if opportunity_score >= 60:
        if current_price < analysis["avg_price"] or PATTERN_PANIC in [p["type"] for p in analysis["patterns"]]:
