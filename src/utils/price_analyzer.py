...
def calculate_price_statistics(
    ...
    try:
        ...
        weighted_avg = sum(p * v for p, v in zip(prices, volumes)) / sum(volumes) if sum(volumes) > 0 else 0
...
async def calculate_price_trend(
    ...
    # Определяем тренд
    increases = sum(1 for i in range(1, len(period_prices)) if period_prices[i] > period_prices[i-1])
    decreases = sum(1 for i in range(1, len(period_prices)) if period_prices[i] < period_prices[i-1])
...
async def find_undervalued_items(
    ...
    try:
        ...
        if not items or "objects" not in items:
            logger.warning(f"Не удалось получить предметы для анализа")
            return []
...
async def get_investment_recommendations(
    ...
    for item in undervalued_items:
        ...
        if (supply_demand["liquidity"] >= min_liquidity and 
