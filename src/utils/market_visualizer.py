...
class MarketVisualizer:
    ...
    async def create_price_chart(
        ...
        # Format main axis
        ax1.set_title(f"{item_name} - Price History", fontsize=14, color=self.text_color)
        ...
        if include_volume and 'volume' in df.columns:
            ...
            ax2.bar(df.index, df['volume'], color=self.volume_color, alpha=0.7, width=0.8)
        ...
        # Save to buffer
        buf = io.BytesIO()
        fig.tight_layout(rect=[0, 0.03, 1, 0.95])  # Adjust layout to make room for stats text
        fig.savefig(buf, format='png', bbox_inches='tight')
        plt.close(fig)
        buf.seek(0)
        ...
        for i, item in enumerate(items_data):
            ...
            if not df.empty:
                ...
                if first_price > 0:
                    ...
                    ax.plot(df.index, normalized, linewidth=2, label=item.get('title', f'Item {i+1}'))
    ...
    async def create_pattern_visualization(
        ...
        # Format the chart
        ax.set_title(f"{item_name} - Pattern Detection", fontsize=14, color=self.text_color)
        ax.grid(linestyle='--', alpha=0.3, color=self.grid_color)
        ...
        pattern_types = set(area['pattern'] for area in pattern_areas)
        handles = [Rectangle((0, 0), 1, 1, color=self.highlight_color, alpha=0.3) for _ in pattern_types]
    ...
    async def create_market_summary_image(
        ...
        # Draw current price
        draw.text((20, 60), f"Current Price: ${current_price:.2f}", font=font_medium, fill=text_color)
        ...
        trend_color = (0, 200, 100) if trend == "up" else (255, 80, 80) if trend == "down" else (170, 170, 170)
        draw.text((20, y_pos), f"Trend: {trend.title()}", font=font_medium, fill=trend_color)
        y_pos += 30
        ...
        price_24h_color = (0, 200, 100) if price_change_24h > 0 else (255, 80, 80) if price_change_24h < 0 else text_color
        price_7d_color = (0, 200, 100) if price_change_7d > 0 else (255, 80, 80) if price_change_7d < 0 else text_color
        
        draw.text((20, y_pos), f"24h Change: {price_change_24h:+.2f}%", font=font_small, fill=price_24h_color)
        y_pos += 25
        draw.text((20, y_pos), f"7d Change: {price_change_7d:+.2f}%", font=font_small, fill=price_7d_color)
        y_pos += 35
        
        # Volatility
        volatility = analysis.get('volatility', 'low')
        volatility_color = (170, 170, 170) if volatility == "low" else (255, 170, 0) if volatility == "medium" else (255, 80, 80)
        draw.text((20, y_pos), f"Volatility: {volatility.title()}", font=font_small, fill=volatility_color)
        y_pos += 35
        ...
        if support is not None:
            draw.text((20, y_pos), f"Support: ${support:.2f}", font=font_small, fill=text_color)
            y_pos += 25
            
        if resistance is not None:
            draw.text((20, y_pos), f"Resistance: ${resistance:.2f}", font=font_small, fill=text_color)
            y_pos += 35
        
        # Patterns detected
        patterns = analysis.get('patterns', [])
        if patterns:
            draw.text((20, y_pos), "Patterns Detected:", font=font_small, fill=text_color)
            y_pos += 25
            ...
            for i, pattern in enumerate(patterns[:3]):  # Show top 3 patterns
                ...
                draw.text((30, y_pos), f"• {pattern_type} ({confidence:.0f}%)", font=font_small, fill=text_color)
                y_pos += 25
        
        # Add timestamp
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        draw.text((width - 200, height - 30), f"Generated: {timestamp}", font=font_small, fill=text_color)
    ...
    def add_support_resistance(self, ax, df: pd.DataFrame) -> None:
        ...
        for i in range(window_size, len(df) - window_size):
            ...
            if df['price'].iloc[i] == df['min'].iloc[i] and df['price'].iloc[i] < df['price'].iloc[i-1] and df['price'].iloc[i] < df['price'].iloc[i+1]:
                support_points.append((df.index[i], df['price'].iloc[i]))
                
            # Resistance point (local maximum)
            if df['price'].iloc[i] == df['max'].iloc[i] and df['price'].iloc[i] > df['price'].iloc[i-1] and df['price'].iloc[i] > df['price'].iloc[i+1]:
        ...
        for date, price in support_points:
            ax.axhline(y=price, color=self.up_color, linestyle='--', alpha=0.5, linewidth=1)
            
        for date, price in resistance_points:
            ax.axhline(y=price, color=self.down_color, linestyle='--', alpha=0.5, linewidth=1) 