...
class MarketVisualizer:
        ...
        for i, item in enumerate(items_data):
            ...
            if not df.empty:
                ...
                if first_price > 0:
                    ...
                    ax.plot(
                        df.index, normalized, linewidth=2, label=item.get("title", f"Item {i+1}")
                    )
    ...
    async def create_market_summary_image(
        ...
        # Draw current price
        draw.text(
            (20, 60), f"Current Price: ${current_price:.2f}", font=font_medium, fill=text_color
        )
        ...
        y_pos += 25
        draw.text(
            (20, y_pos), f"7d Change: {price_change_7d:+.2f}%", font=font_small, fill=price_7d_color
        )
        y_pos += 35
        ...
        draw.text(
            (20, y_pos), f"Volatility: {volatility.title()}", font=font_small, fill=volatility_color
        )
        y_pos += 35
        ...
        if resistance is not None:
            draw.text(
                (20, y_pos), f"Resistance: ${resistance:.2f}", font=font_small, fill=text_color
            )
            y_pos += 35
        ...
        draw.text(
            (width - 200, height - 30), f"Generated: {timestamp}", font=font_small, fill=text_color
        )
