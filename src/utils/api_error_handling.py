...
class APIError(Exception):
    ...
    def __init__(
        self, message: str, status_code: int = 0, response_data: dict[str, Any] | None = None
    ):
        ...
        if isinstance(self.response_data, dict):
            ...
            return str(
                self.response_data.get(
                    "code",
                    self.response_data.get("error_code", self.response_data.get("error", "")),
                )
            )
        return ""
...
async def handle_response(response: ClientResponse) -> dict[str, Any]:
    ...
    if 500 <= status < 600:
        raise ServerError(
            f"Серверная ошибка API (код {status})",
            status,
            data,
        )
    if status == 400:
        ...
        raise BadRequestError(
            f"Неверный запрос: {error_message}",
            status,
            data,
        )
    # Если есть специальный класс для этой ошибки DMarket
...
async def retry_request(
    ...
    **kwargs,
    ...
    raise APIError(
        f"Неизвестная ошибка после {attempt} попыток: {last_error!s}",
        0,
    )
