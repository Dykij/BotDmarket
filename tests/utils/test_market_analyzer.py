...
def sample_price_history():
    ...
    return [
        {"price": 10.0, "timestamp": (datetime.now() - timedelta(days=6)).timestamp()},
        {"price": 10.5, "timestamp": (datetime.now() - timedelta(days=5)).timestamp()},
        {"price": 11.2, "timestamp": (datetime.now() - timedelta(days=4)).timestamp()},
        {"price": 11.8, "timestamp": (datetime.now() - timedelta(days=3)).timestamp()},
        {"price": 11.5, "timestamp": (datetime.now() - timedelta(days=2)).timestamp()},
        {"price": 12.0, "timestamp": (datetime.now() - timedelta(days=1)).timestamp()},
        {"price": 12.5, "timestamp": datetime.now().timestamp()},
    ]
...
def downtrend_price_history():
    ...
    return [
        {"price": 15.0, "timestamp": (datetime.now() - timedelta(days=6)).timestamp()},
        {"price": 14.5, "timestamp": (datetime.now() - timedelta(days=5)).timestamp()},
        {"price": 14.0, "timestamp": (datetime.now() - timedelta(days=4)).timestamp()},
        {"price": 13.2, "timestamp": (datetime.now() - timedelta(days=3)).timestamp()},
        {"price": 12.8, "timestamp": (datetime.now() - timedelta(days=2)).timestamp()},
        {"price": 12.3, "timestamp": (datetime.now() - timedelta(days=1)).timestamp()},
        {"price": 12.0, "timestamp": datetime.now().timestamp()},
    ]
...
def volatile_price_history():
    ...
    return [
        {"price": 10.0, "timestamp": (datetime.now() - timedelta(days=6)).timestamp()},
        {"price": 12.5, "timestamp": (datetime.now() - timedelta(days=5)).timestamp()},
        {"price": 9.8, "timestamp": (datetime.now() - timedelta(days=4)).timestamp()},
        {"price": 11.5, "timestamp": (datetime.now() - timedelta(days=3)).timestamp()},
        {"price": 10.2, "timestamp": (datetime.now() - timedelta(days=2)).timestamp()},
        {"price": 13.0, "timestamp": (datetime.now() - timedelta(days=1)).timestamp()},
        {"price": 11.5, "timestamp": datetime.now().timestamp()},
    ]
...
class TestMarketOpportunity:
    ...
    async def test_analyze_market_opportunity_with_patterns(self, sample_item_data):
        ...
        fomo_history = [
            {"price": 10.0, "timestamp": (datetime.now() - timedelta(days=5)).timestamp()},
            {"price": 10.5, "timestamp": (datetime.now() - timedelta(days=4)).timestamp()},
            {"price": 11.2, "timestamp": (datetime.now() - timedelta(days=3)).timestamp()},
            {"price": 12.8, "timestamp": (datetime.now() - timedelta(days=2)).timestamp()},
