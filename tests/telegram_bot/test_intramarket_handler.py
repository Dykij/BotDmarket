...
class TestHandleIntramarketCallback:
    ...
    async def test_handle_anomaly_callback(self, mock_anomalies, update, context):
        ...
        with patch('src.telegram_bot.handlers.intramarket_arbitrage_handler.pagination_manager') as mock_pagination:
            with patch('src.telegram_bot.handlers.intramarket_arbitrage_handler.format_paginated_results', return_value="Formatted results"):
                # Вызываем тестируемую функцию
                ...
                assert "Formatted results" == last_call_args[0]
