"""
Тесты для функций автоматического арбитража в bot_v2.py
"""
...
async def test_handle_pagination_next(mock_markup, mock_pagination_manager, mock_format_results, mock_query, mock_context):
    """Тест функции для обработки пагинации - следующая страница."""
    # Импортируем функцию из auto_arbitrage, а не напрямую из bot_v2
    from src.telegram_bot.auto_arbitrage import handle_pagination
    
    # Настройка моков
    mock_pagination_manager.next_page.return_value = (
        [{"title": "Test Item"}],  # items
        1,  # current_page
        2   # total_pages
    )
    ...
    # Настройка данных коллбэка
    mock_query.data = "paginate:next:auto_medium"
...
@pytest.mark.asyncio
@patch("src.telegram_bot.bot_v2.format_paginated_results")
@patch("src.telegram_bot.bot_v2.pagination_manager")
@patch("src.telegram_bot.bot_v2.InlineKeyboardMarkup")
async def test_handle_pagination_prev(mock_markup, mock_pagination_manager, mock_format, mock_query, mock_context):
    """Тест функции для обработки пагинации - предыдущая страница."""
    # Импортируем функцию из auto_arbitrage, а не напрямую из bot_v2
    # из-за синтаксических ошибок в bot_v2.py
    from src.telegram_bot.auto_arbitrage import handle_pagination
    
    # Настройка моков
    mock_pagination_manager.prev_page.return_value = (
        [{"title": "Test Item"}],  # items
        0,  # current_page
        2   # total_pages
    )
    ...
    # Настройка данных коллбэка
    mock_query.data = "paginate:prev:auto_medium"
    
    # Вызов тестируемой функции
    await handle_pagination(mock_query, mock_context)
...
async def test_show_auto_stats_with_items(mock_get_keyboard, mock_pagination_manager, mock_format, mock_query, mock_context):
    ...
    mock_get_keyboard.return_value.inline_keyboard = [
        [MagicMock()]
    ]
...
@pytest.mark.asyncio
@patch("src.telegram_bot.bot_v2.get_back_to_arbitrage_keyboard")
@patch("src.telegram_bot.bot_v2.pagination_manager")
async def test_show_auto_stats_no_items(mock_pagination_manager, mock_get_keyboard, mock_query, mock_context):
    ...
    assert "нет данных" in kwargs.get("text", "").lower() or "не найдено" in kwargs.get("text", "").lower()
